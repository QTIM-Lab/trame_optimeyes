...
"""
    ...
    parser.add_argument(
        "language", help="The desired language, one of: CSharp, Cxx, Java, Python."
    )
    parser.add_argument(
        "-a",
        "--all_values",
    ...
    parser.add_argument(
        "-n", "--number", type=int, default=5, help="The maximum number of examples."
    )
    parser.add_argument(
        "-m",
        "--md",
...
class Links:
    ...
    xref_url: str = "https://raw.githubusercontent.com/Kitware/vtk-examples/gh-pages/src/Coverage/vtk_vtk-examples_xref.json"
...
def get_examples(d, vtk_class, lang, all_values=False, number=5, md_fmt=False):
    ...
    if md_fmt:
        links = [
            f'[{s.rsplit("/", 1)[1]}]({s})' for s in sorted(map(itemgetter(1), samples))
        ]
    else:
...
def main():
    ...
    total_number, examples = get_examples(
        xref_dict, vtk_class, language, all_values=all_values, number=number, md_fmt=md
    )
    if examples:
